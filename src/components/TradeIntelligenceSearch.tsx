...
export const TradeIntelligenceSearch: React.FC<TradeIntelligenceSearchProps> = ({
  ...
  return (
    <div id="trade-intelligence-explorer" className="flex-1 flex flex-col min-h-0 overflow-hidden space-y-4 text-right">
      ...
      <div className="bg-gradient-to-l from-slate-900 via-slate-800 to-[#0F172A] text-white p-5 rounded-2xl border border-slate-700/60 shadow-xs flex-shrink-0 relative overflow-hidden">
        <div className="absolute -top-10 -left-10 w-60 h-60 bg-blue-500/10 rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="bg-blue-600 text-white text-[11px] font-bold px-2 py-0.5 rounded-md">
                موتور هوش تجاری چندگانه
              </span>
              <span className="text-slate-400 text-xs font-medium">
                استعلام هم‌زمان از ۹ پایگاه معتبر بین‌المللی و گمرکی
              </span>
            </div>
            <h2 className="text-base md:text-lg font-bold text-white tracking-tight">
              کاوشگر اسناد، تأمین‌کنندگان، قیمت روز و مقررات گمرکی
            </h2>
            <p className="text-xs text-slate-300 max-w-3xl leading-relaxed">
              دسترسی به داده‌های ساختاریافته از <strong className="text-blue-300">ITC Trade Map</strong>، پایگاه بارنامه‌های دریایی <strong className="text-cyan-300">ImportYeti</strong>، پایگاه کارخانجات چین <strong className="text-red-300">Baidu B2B</strong>، موتور پژوهش اسناد <strong className="text-purple-300">Perplexity</strong>، پایشگر قیمت زنده <strong className="text-amber-300">Apify</strong>، زنجیره تأمین <strong className="text-indigo-300">Panjiva</strong> و سامانه ارزش <strong className="text-emerald-300">TSC گمرک ایران</strong>.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-colors shadow-xs flex items-center gap-1.5"
            >
              <FileCheck2 className="w-4 h-4" />
              <span>شروع پرونده ارزیابی جدید</span>
            </button>
          </div>
        </div>
        ...
        <div className="mt-4 relative">
          <input
            ...
            placeholder="جستجوی زنده کالا، کد تعرفه ۸ رقمی (مثلاً 8541.43)، نام کارخانه، کشور مبدأ، اصطلاح انگلیسی یا کلیدواژه..."
            className="w-full bg-slate-950/90 border border-slate-600 text-white rounded-xl py-3 pr-11 pl-28 text-xs md:text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 font-sans shadow-inner"
          />
          ...
          <div className="absolute left-3 top-2.5 flex items-center gap-1.5">
            {searchQuery && (
              <button
                ...
                className="text-xs text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 px-2.5 py-1 rounded-md transition-colors flex items-center gap-1"
            ...
            <span className="text-[11px] font-mono text-blue-300 bg-blue-900/60 border border-blue-700/50 px-2 py-0.5 rounded">
              {filteredRecords.length} نتیجه
            </span>
          </div>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2 overflow-x-auto pb-1 text-xs">
          ...
          {quickPills.map((pill) => (
            <button
              ...
              className="bg-slate-800/90 hover:bg-slate-700 text-slate-200 text-[11px] font-medium px-2.5 py-1 rounded-lg border border-slate-700 whitespace-nowrap transition-colors"
            >
              {pill}
            </button>
          ))}
