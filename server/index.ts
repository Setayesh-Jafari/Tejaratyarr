/**
 * تجارت‌یار — سرور برنامه (Express)
 * ------------------------------------------------------------------
 * توسعه ۱۴۰۵: API کامل CRUD + لایه هوش مصنوعی Gemini
 * توسعه: پرت ۳۰۰۱ (پیش‌فرض)، در محیط تولید پوشه dist را سرو می‌کند.
 */
import express, { Request, Response, NextFunction } from 'express';
import path from 'path';
import fs from 'fs';
import { db, uid } from './db';
import { hsSuggest, supplierCheck, isAiEnabled, modelName } from './ai';
import type { InventoryUnit, SupplierRecord, TradeAssessmentDossier, AppSettings } from '../src/types';

const app = express();
app.use(express.json({ limit: '2mb' }));

/* درخواست‌ها فقط از مرورگر کاربر می‌آیند؛ CORS برای توسعه محلی */
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ تجارت‌یار API روی پورت ${PORT} فعال شد — ${isAiEnabled() ? `هوش مصنوعی فعال (${modelName()})` : 'هوش مصنوعی غیرفعال (بدون GEMINI_API_KEY)'}`);
});
